...
export function generateTypeFixOptions(issue: Issue): FixOption[] {
  ...
  options.push({
    ...
    sqlTemplate: `ALTER TABLE \`${table}\` MODIFY COLUMN \`${column}\` ${baseType};`,
    isRecommended: false,
    effort: 'low',
    risk: 'low',
    impact: '부동소수점 근사값 동작은 동일하게 유지됨',
    reversibility: 'reversible',
    rollbackTemplate: `ALTER TABLE \`${table}\` MODIFY COLUMN \`${column}\` ${columnType};`,
  });
  ...
  options.push({
    ...
    sqlTemplate: `ALTER TABLE \`${table}\` MODIFY COLUMN \`${column}\` DECIMAL(${precision},${scale});`,
    isRecommended: false,
    effort: 'low',
    risk: 'medium',
