...
export function generateAuthFixOptions(issue: Issue): FixOption[] {
  ...
  const escapedUser = escapeIdentifier(userName);
  ...
  if (issue.id === 'mysql_native_password') {
    options.push({
      ...
      sqlTemplate: `ALTER USER ${escapedUser}@'%' IDENTIFIED WITH caching_sha2_password BY 'NEW_PASSWORD_HERE';`,
      isRecommended: false,
      effort: 'medium',
      risk: 'medium',
      impact: '클라이언트가 caching_sha2_password를 지원해야 함. PHP < 7.4, Python MySQL Connector < 8.0 등 구버전 주의.',
      reversibility: 'reversible',
      rollbackTemplate: `ALTER USER ${escapedUser}@'%' IDENTIFIED WITH mysql_native_password BY 'PASSWORD_HERE';`,
    });
    ...
    options.push({
      ...
      reversibility: 'reversible',
      rollbackTemplate: `ALTER USER ${escapedUser}@'%' IDENTIFIED WITH sha256_password BY 'PASSWORD_HERE';`,
    });
    ...
    options.push({
      ...
      sqlTemplate: `ALTER USER ${escapedUser}@'%' IDENTIFIED WITH authentication_webauthn;`,
      isRecommended: false,
      effort: 'medium',
      risk: 'low',
      impact: 'WebAuthn 호환 키가 필요합니다.',
      reversibility: 'reversible',
      rollbackTemplate: `ALTER USER ${escapedUser}@'%' IDENTIFIED WITH authentication_fido;`,
    });
  }
