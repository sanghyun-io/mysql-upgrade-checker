 ...
 *   - "app_user"              → { user: "app_user", host: "%" }
 *   - "app_user@localhost"    → { user: "app_user", host: "localhost" }
 *   - "'admin'@'10.0.0.1'"   → { user: "admin",    host: "10.0.0.1" }
 *   - "admin@10.0.0.1"       → { user: "admin",    host: "10.0.0.1" }
 */
export function parseUserHost(userName: string): { user: string; host: string } {
  // Handle 'user'@'host' format (with or without surrounding quotes)
  const match = userName.match(/^'?([^'@]+)'?\s*@\s*'?([^']+)'?$/);
  if (match) {
    return { user: match[1], host: match[2] };
  }
  // Simple user name without host — default to wildcard host
...
export function generateAuthFixOptions(issue: Issue): FixOption[] {
  ...
  if (issue.id === 'mysql_native_password') {
    options.push({
      ...
      sqlTemplate: `ALTER USER ${userAtHost} IDENTIFIED WITH caching_sha2_password BY 'NEW_PASSWORD_HERE';`,
      isRecommended: false,
      effort: 'medium',
      risk: 'medium',
      impact: '클라이언트가 caching_sha2_password를 지원해야 함. PHP < 7.4, Python MySQL Connector < 8.0 등 구버전 주의.',
      reversibility: 'reversible',
      rollbackTemplate: `ALTER USER ${userAtHost} IDENTIFIED WITH mysql_native_password BY 'PASSWORD_HERE';`,
    });
    ...
    options.push({
      ...
      reversibility: 'reversible',
      rollbackTemplate: `ALTER USER ${userAtHost} IDENTIFIED WITH sha256_password BY 'PASSWORD_HERE';`,
    });
    ...
    options.push({
      ...
      sqlTemplate: `ALTER USER ${userAtHost} IDENTIFIED WITH authentication_webauthn;`,
      isRecommended: false,
      effort: 'medium',
      risk: 'low',
      impact: 'WebAuthn 호환 키가 필요합니다.',
      reversibility: 'reversible',
      rollbackTemplate: `ALTER USER ${userAtHost} IDENTIFIED WITH authentication_fido;`,
    });
  }
